...
for (
  var eventTypes$jscomp$inline_153 = "change selectionchange textInput compositionstart compositionend compositionupdate".split(
      " "
    ),
    i$jscomp$inline_155 = 0;
  i$jscomp$inline_155 < eventTypes$jscomp$inline_153.length;
  i$jscomp$inline_155++
)
  eventPriorities.set(eventTypes$jscomp$inline_153[i$jscomp$inline_155], 0);
...
function flushPassiveEffectsImpl() {
  ...
  if (runAllPassiveEffectDestroysBeforeCreates) {
    ...
    for (create = root.current.firstEffect; null !== create; ) {
      try {
        if (((unmountEffects = create), 0 !== (unmountEffects.effectTag & 512)))
  ...
  switch (workInProgress.tag) {
    ...
    case 16:
      a: {
        ...
        getDerivedStateFromProps = workInProgress.tag = resolveLazyComponentTag(
          newProps
        );
    ...
    case 10:
      a: {
        ...
        getDerivedStateFromProps = newProps.value;
        var context = workInProgress.type._context;
        push(valueCursor, context._currentValue);
        context._currentValue = getDerivedStateFromProps;
        if (null !== resolvedProps)
          if (
            ((context = resolvedProps.value),
            (getDerivedStateFromProps = objectIs(
              context,
              getDerivedStateFromProps
            )
              ? 0
              ...
              if (null !== list) {
                ...
                for (
                  ...
                  if (
                    ...
                    0 !== (dependency.observedBits & getDerivedStateFromProps)
                  ) {
                    1 === resolvedProps.tag &&
    ...
    case 9:
      return (
        ...
        (updateExpirationTime = getDerivedStateFromProps.children),
        prepareToReadContext(workInProgress, renderExpirationTime),
        (newProps = readContext(
          newProps,
          getDerivedStateFromProps.unstable_observedBits
        )),
    ...
    case 14:
      return (
        (newProps = workInProgress.type),
        (getDerivedStateFromProps = resolveDefaultProps(
          newProps,
          workInProgress.pendingProps
        )),
        (getDerivedStateFromProps = resolveDefaultProps(
...
function updateContainer(element, container, parentComponent, callback) {
  ...
  scheduleWork(parentComponent, currentTime);
  return currentTime;
}
