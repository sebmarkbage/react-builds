...
function executeDispatchesInOrder(event) {
  ...
  if (Array.isArray(dispatchListeners))
    for (var previousInstance, i = 0; i < dispatchListeners.length; i++) {
      ...
      if (instance !== previousInstance && event.isPropagationStopped()) break;
...
function executeDispatchesAndReleaseTopLevel(e) {
  e &&
    (executeDispatchesInOrder(e), e.isPersistent() || e.constructor.release(e));
}
...
for (
  var eventTypes$jscomp$inline_148 = "change selectionchange textInput compositionstart compositionend compositionupdate".split(
      " "
    ),
    i$jscomp$inline_150 = 0;
  i$jscomp$inline_150 < eventTypes$jscomp$inline_148.length;
  i$jscomp$inline_150++
)
  eventPriorities.set(eventTypes$jscomp$inline_148[i$jscomp$inline_150], 0);
var capturePhaseEvents = new Set(
  ...
  eventTargetEventListenerStore = new ("function" === typeof WeakMap
    ? WeakMap
    : Map)();
function listenToTopLevelEvent(
  ...
  var listenerMapKey =
      void 0 === capture
        ? topLevelType
        : topLevelType + "_" + (capture ? "capture" : "bubble"),
    listenerEntry = listenerMap.get(listenerMapKey),
    shouldUpgrade =
      void 0 === listenerEntry
        ? !1
        : (!0 !== passive && listenerEntry.passive) ||
          (!1 === passive && void 0 === listenerEntry.passive)
        ? !0
        : !1;
  if (void 0 === listenerEntry || shouldUpgrade)
    (capture =
      void 0 === capture ? capturePhaseEvents.has(topLevelType) : capture),
      shouldUpgrade &&
      ...
      (topLevelType = addTrappedEventListener(
        targetContainer,
        topLevelType,
        capture,
        !1,
...
function dispatchEventForPluginEventSystem(
  ...
  if (null !== targetContainer)
    if (eventTargetEventListenerStore.has(targetContainer))
      (ancestorInst = null), (eventSystemFlags$jscomp$0 |= 128);
    else if (null !== targetInst$jscomp$0)
      for (var node = targetInst$jscomp$0; ; ) {
        if (null === node) return;
        if (3 === node.tag || 4 === node.tag) {
          targetInst$jscomp$0 = node.stateNode.containerInfo;
          if (
            targetInst$jscomp$0 === targetContainer ||
            (targetInst$jscomp$0.nodeType === COMMENT_NODE &&
              targetInst$jscomp$0.parentNode === targetContainer)
          )
            break;
          if (4 === node.tag)
            for (node = node.return; null !== node; ) {
              if (3 === node.tag || 4 === node.tag) {
                var grandContainer = node.stateNode.containerInfo;
                if (
          ...
          targetInst$jscomp$0 = getClosestInstanceFromNode(targetInst$jscomp$0);
          if (null === targetInst$jscomp$0) return;
          node = ancestorInst = targetInst$jscomp$0;
        } else node = node.return;
      }
  batchedEventUpdates(function() {
    for (
      var eventSystemFlags = eventSystemFlags$jscomp$0,
        targetInst = ancestorInst,
      ...
      void 0 !== possiblePlugin &&
        ...
        isArray(possiblePlugin)
          ? syntheticEvents.push.apply(syntheticEvents, possiblePlugin)
          : null != possiblePlugin && syntheticEvents.push(possiblePlugin));
    }
    for (
      eventSystemFlags = 0;
      eventSystemFlags < syntheticEvents.length;
      eventSystemFlags++
    )
      (targetInst = syntheticEvents[eventSystemFlags]),
        executeDispatchesInOrder(targetInst),
...
function eagerlyTrapReplayableEvents(container, document) {
  ...
  discreteReplayableEvents.forEach(function(topLevelType) {
    enableModernEventSystem &&
      listenToTopLevelEvent(topLevelType, container, listenerMapForContainer);
  ...
  if (enableModernEventSystem) {
    ...
    for (
      rootContainerInstance =
        ...
        listenerMap = getListenerMapForElement(rootContainerInstance),
        registrationName = registrationNameDependencies[registrationName],
        i = 0;
      i < registrationName.length;
      i++
    )
...
var previousScreenX = 0,
  ...
  EnterLeaveEventPlugin = {
    ...
    extractEvents: function(
      ...
      if (isOverEvent && 0 === (eventSystemFlags & 32)) {
        var related = nativeEvent.relatedTarget || nativeEvent.fromElement;
        if (related)
          if (enableModernEventSystem) {
            if (null !== getClosestInstanceFromNode(related)) return null;
          } else return null;
      }
      ...
      if (isOutEvent) {
        if (
          ...
          (targetInst = (targetInst =
            ...
            : null),
          null !== targetInst &&
            ((related = getNearestMountedFiber(targetInst)),
            targetInst !== related ||
      ...
      if (nativeEventTarget && eventTypePrefix)
        a: {
          ...
          for (; 0 < topLevelType - isOutEvent; )
            (eventInterface = getParent(eventInterface)), topLevelType--;
      ...
      for (
        ...
        topLevelType = nativeEventTarget.alternate;
        if (null !== topLevelType && topLevelType === enterEventType) break;
      ...
      for (
        ...
        topLevelType = eventTypePrefix.alternate;
        if (null !== topLevelType && topLevelType === enterEventType) break;
      ...
      return enableModernEventSystem || 0 !== (eventSystemFlags & 64)
...
var getInstanceFromNodeImpl$jscomp$inline_365 = getInstanceFromNode$2;
getFiberCurrentPropsFromNode = getFiberCurrentPropsFromNode$1;
getInstanceFromNode = getInstanceFromNodeImpl$jscomp$inline_365;
    ...
    if (
      ...
      return (
        0 === (workInProgress.mode & 2)
          ...
          : current.data === SUSPENSE_FALLBACK_START_DATA
...
function commitLifeCycles(finishedRoot, current, finishedWork) {
  switch (finishedWork.tag) {
    ...
    case 15:
    case 22:
      finishedRoot = Layout | HasEffect;
      current = finishedWork.updateQueue;
      current = null !== current ? current.lastEffect : null;
      if (null !== current) {
        var effect = (current = current.next);
        do {
          if ((effect.tag & finishedRoot) === finishedRoot) {
            var create = effect.create;
            effect.destroy = create();
          }
          effect = effect.next;
        } while (effect !== current);
      }
...
function commitWork(current, finishedWork) {
  switch (finishedWork.tag) {
    ...
    case 5:
      ...
      if (null != tag) {
        ...
        if (null !== updatePayload) {
          tag[internalEventHandlersKey] = effect;
          "input" === type &&
            "radio" === effect.type &&
            null != effect.name &&
            updateChecked(tag, effect);
          isCustomComponent(type, current);
          for (
            var isCustomComponentTag = isCustomComponent(type, effect), i = 0;
            ...
            "style" === propKey
              ? setValueForStyles(tag, propValue)
              : "dangerouslySetInnerHTML" === propKey
              ? setInnerHTML(tag, propValue)
              : "children" === propKey
              ? setTextContent(tag, propValue)
              : setValueForProperty(
                  tag,
                  propKey,
                  propValue,
          ...
          switch (type) {
            case "input":
              updateWrapper(tag, effect);
              break;
            case "textarea":
              updateWrapper$1(tag, effect);
              break;
            case "select":
              (type = tag._wrapperState.wasMultiple),
                (tag._wrapperState.wasMultiple = !!effect.multiple),
                (updatePayload = effect.value),
                null != updatePayload
                  ? updateOptions(tag, !!effect.multiple, updatePayload, !1)
                  : type !== !!effect.multiple &&
        ...
        tag = effect.DEPRECATED_flareListeners;
        current.DEPRECATED_flareListeners !== tag &&
          updateDeprecatedEventListeners(tag, finishedWork, null);
      }
      return;
    case 6:
    ...
    case 12:
      return;
    case 13:
      tag = finishedWork.memoizedState;
      effect = finishedWork;
      null === tag
        ? (current = !1)
        : ((current = !0),
      ...
      if (null !== effect)
        a: for (type = effect; ; ) {
                ...
                if ((i = null !== isCustomComponentTag))
                  b: {
                    ...
                    for (
                      ...
                      if (5 === propKey.tag && propKey.stateNode === i) {
                        i = !0;
                        break b;
                      }
          ...
          if (type === effect) break;
          for (; null === type.sibling; ) {
            if (null === type.return || type.return === effect) break a;
            type = type.return;
          }
      ...
      null !== tag &&
        ((tag = finishedWork.memoizedProps.suspenseCallback),
        "function" === typeof tag &&
          ((effect = finishedWork.updateQueue),
          null !== effect && tag(new Set(effect))));
    ...
    case 21:
      ...
      tag = finishedWork.memoizedProps;
      effect = tag.DEPRECATED_flareListeners;
      ((null !== current ? current.memoizedProps : tag)
        .DEPRECATED_flareListeners === effect &&
        null !== current) ||
...
var PossiblyWeakMap$2 = "function" === typeof WeakMap ? WeakMap : Map;
