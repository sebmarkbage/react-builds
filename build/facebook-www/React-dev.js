...
function warnIfStringRefCannotBeAutoConverted$1(config, self) {
  {
    if (
      ...
      var componentName = getComponentName(ReactCurrentOwner$1.current.type);
      ...
      if (!didWarnAboutStringRefs$1[componentName]) {
        error(
          'Component "%s" contains the string ref "%s". ' +
            ...
            "Learn more about using refs safely here: " +
            "https://fb.me/react-strict-mode-string-ref",
          getComponentName(ReactCurrentOwner$1.current.type),
          config.ref
        );
...
function getDeclarationErrorAddendum$1() {
  {
    if (ReactCurrentOwner$2.current) {
      var name = getComponentName(ReactCurrentOwner$2.current.type);
...
function validateExplicitKey$1(element, parentType) {
  {
    ...
    ReactDebugCurrentFrame$1.setCurrentlyValidatingElement(element);
    ...
    ReactDebugCurrentFrame$1.setCurrentlyValidatingElement(null);
  }
}
/**
 * Ensure that every element either is passed in a static location, in an
...
function validatePropTypes$1(element) {
  {
    ...
    if (propTypes) {
      ...
      propTypesMisspellWarningShown$1 = true;
...
function validateFragmentProps$1(fragment) {
  {
    ReactDebugCurrentFrame$1.setCurrentlyValidatingElement(fragment);
    var keys = Object.keys(fragment.props);
