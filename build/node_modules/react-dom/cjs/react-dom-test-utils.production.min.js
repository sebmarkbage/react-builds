...
function U(){}function oa(a,b){if(!a)return[];a=u(a);if(!a)return[];for(var c=a,e=[];;){if(5===c.tag||6===c.tag||1===c.tag||0===c.tag){var d=c.stateNode;b(d)&&e.push(d)}if(c.child)c.child.return=c,c=c.child;else{if(c===a)return e;for(;!c.sibling;){if(!c.return||c.return===a)return e;c=c.return}c.sibling.return=c.return;c=c.sibling}}}
...
b){return W.isCompositeComponent(a)?a._reactInternalFiber.type===b:!1},findAllInRenderedTree:function(a,b){V(a,"findAllInRenderedTree");return a?oa(a._reactInternalFiber,b):[]},scryRenderedDOMComponentsWithClass:function(a,b){V(a,"scryRenderedDOMComponentsWithClass");return W.findAllInRenderedTree(a,function(a){if(W.isDOMComponent(a)){var c=a.className;"string"!==typeof c&&(c=a.getAttribute("class")||"");var d=c.split(/\s+/);if(!Array.isArray(b)){if(void 0===b)throw Error(p(11));b=b.split(/\s+/)}return b.every(function(a){return-1!==
...
L.current;var e=K.current;L.current=!0;K.current=!0;try{var d=fa(a)}catch(f){throw b(),f;}if(null!==d&&"object"===typeof d&&"function"===typeof d.then)return{then:function(a,e){d.then(function(){1<P||!0===M&&!0===c?(b(),a()):O(function(c){b();c?e(c):a()})},function(a){b();e(a)})}};try{1!==P||!1!==M&&!1!==c||N(),b()}catch(f){throw b(),f;}return{then:function(a){a()}}}};
function pa(a){return function(b,c){if(k.isValidElement(b))throw Error(p(228));if(W.isCompositeComponent(b))throw Error(p(229));var e=S[a],d=new U;d.target=b;d.type=a.toLowerCase();var f=ia(b),h=new x(e,f,d,b);h.persist();g(h,c);e.phasedRegistrationNames?ja(h):ka(h);m.unstable_batchedUpdates(function(){la(b);na(h)});ma()}}W.Simulate={};for(var X in S)W.Simulate[X]=pa(X);
function qa(a,b){return function(c,e){var d=new U(a);g(d,e);W.isDOMComponent(c)?(c=ha(c),d.target=c,T(b,1,document,d)):c.tagName&&(d.target=c,T(b,1,document,d))}}
...
["touchstart","touchStart"],[ca,"transitionEnd"],["volumechange","volumeChange"],["waiting","waiting"],["wheel","wheel"]].forEach(function(a){var b=a[1];W.SimulateNative[b]=qa(b,a[0])});var Y={__proto__:null,"default":W},Z=Y&&Y["default"]||Y;module.exports=Z.default||Z;
